<div>
                        ...
                        <table class="min-w-full divide-y divide-gray-300 dark:divide-gray-700">
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700 bg-white dark:bg-gray-900">
                                <?php $__empty_1 = true; $__currentLoopData = $streams; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $stream): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                    <tr wire:key="<?php echo e($stream->id); ?>">
                                        <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 dark:text-white sm:pl-6"><?php echo e($stream->name); ?></td>
                                        <?php if(Auth::user()->isAdmin()): ?>
                                            <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500 dark:text-gray-300"><?php echo e($stream->user->name); ?></td>
                                        <?php endif; ?>
                                        <td class="whitespace-nowrap px-3 py-4 text-sm">
                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-<?php echo e($stream->status_color); ?>-100 text-<?php echo e($stream->status_color); ?>-800 dark:bg-<?php echo e($stream->status_color); ?>-800 dark:text-<?php echo e($stream->status_color); ?>-100">
                                                <?php echo e(ucfirst($stream->status)); ?>

                                            </span>
                                        </td>
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500 dark:text-gray-300"><?php echo e(ucfirst($stream->platform)); ?></td>
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500 dark:text-gray-300">
                                            <?php echo e($stream->is_scheduled ? \Carbon\Carbon::parse($stream->scheduled_at)->format('H:i d/m/Y') : 'Không'); ?>

                                        </td>
                                        <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                                            <button wire:click="edit(<?php echo e($stream->id); ?>)" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-200">Sửa</button>
                                            <button wire:click="confirmDelete(<?php echo e($stream); ?>)" class="ml-4 text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-200">Xóa</button>
                                        </td>
                                    </tr>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                                    <tr>
                                        <td colspan="<?php echo e(Auth::user()->isAdmin() ? '6' : '5'); ?>" class="whitespace-nowrap px-3 py-4 text-sm text-center text-gray-500 dark:text-gray-300">
    ...
    <?php if($showCreateModal): ?>
        <?php echo $__env->make('livewire.partials.stream-form-modal', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    <?php endif; ?>
    
    <!-- Quick Stream Modal -->
    <?php if($showQuickStreamModal): ?>
        <?php echo $__env->make('livewire.partials.quick-stream-modal', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    <?php endif; ?>
...
<?php /**PATH D:\laragon\www\ezstream\resources\views\livewire\stream-manager.blade.php ENDPATH**/ ?>
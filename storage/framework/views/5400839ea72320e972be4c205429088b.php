...
// Force cache refresh - v5.0.1
    function quickUploader() {
        return {
                ...
                try {
                    ...
                    if (!uploadUrlResponse.ok) {
                        ...
                        if (errorData.reason && errorData.details && errorData.solutions) {
                            this.showDetailedErrorFallback(errorData);
                            this.reset();
                            return;
                        }
            ...
            showDetailedErrorFallback(errorData) {
                // Try to use global function first
                if (window.showDetailedErrorModal && typeof window.showDetailedErrorModal === 'function') {
                    window.showDetailedErrorModal(errorData);
                    return;
                }

                // Fallback to custom modal
                this.createErrorModal(errorData);
            },
            createErrorModal(errorData) {
                // Remove existing modal if any
                const existingModal = document.querySelector('.quick-upload-error-modal');
                if (existingModal) {
                ...
                modalOverlay.className = 'quick-upload-error-modal fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[10000] p-4';
                ...
                if (errorData.details) {
                    ...
                    for (const [key, value] of Object.entries(errorData.details)) {
                        const label = this.formatDetailKey(key);
                ...
                modalContent.innerHTML = `
                    <div class="p-6">
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <button onclick="this.closest('.quick-upload-error-modal').remove()" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
                        ...
                        <div class="mt-6 flex justify-end">
                            <button onclick="this.closest('.quick-upload-error-modal').remove()" class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg font-medium">
                                Đóng
                            </button>
                        </div>
                    </div>
